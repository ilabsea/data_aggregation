...
class Provincial extends CI_Controller{
  ...
  function show_risk(){
    ...
    $data = $this->get_confirm_test_as_od("Battambang", $data);
    $data = $this->get_confirm_test_as_province("Battambang", $data);
    $this->load->view('Provincial/risk',$data);
  }

  function get_confirm_test_as_od($province_name, $data){
    $ods = $this->get_od_by_provinces($province_name);
    $current_month = date('m');
    $current_year = date('Y');
    
    $list_months = array_reverse($this->get_list_month($current_month, 6));
    ...
    foreach($ods as $od){
      ...
      for($i=0; $i<6; $i++){
        ...
        $data["graph"][$od["ODname"]][$list_months[$i]]= $this->get_tester_confirm_by_OD($od["ODname"], "$current_year-$current_month-01", "$current_year-$current_month-$end_month_day");
      ...
      $central_line = $tmp_total / 6;
      $avg_moving_range = $total_mv_range /6;
  ...
  function get_confirm_test_as_province($province_name, $data){
    $ods = $this->get_od_by_provinces($province_name);
    $current_month = date('m');
    $current_year = date('Y');
    ...
    for($i=0; $i<6; $i++){
      $end_month_day = $this->day_in_month($current_year, $list_months[$i]);
      $data["graph_province"][$province_name][$list_months[$i]]= $this->get_tester_confirm_by_province($province_name, "$current_year-$current_month-01", "$current_year-$current_month-$end_month_day");
  ...
  function get_tester_confirm_by_province($province_name, $from, $to){
    $this->load->database();
    $result = $this->db->query("select count(*) as total from tblfirsttest INNER JOIN tblconfirm ON tblconfirm.CaseID=tblfirsttest.CaseID INNER JOIN tblcenter ON tblconfirm.Code=tblcenter.Code WHERE tblcenter.Province='$province_name' AND tblfirsttest.DatTest BETWEEN '$from' AND '$to'");
    return $result->result_array();
  }

  function get_tester_confirm_by_OD($od_name, $from, $to){
    $this->load->database();
    $result = $this->db->query("select count(*) as total from tblfirsttest INNER JOIN tblconfirm ON tblconfirm.CaseID=tblfirsttest.CaseID INNER JOIN tblcenter ON tblconfirm.Code=tblcenter.Code WHERE tblcenter.ODname='$od_name' AND tblfirsttest.DatTest BETWEEN '$from' AND '$to';");
    return $result->result_array();
  }
